...
const ExcelProcessor = () => {
  ...
  const createPersonalizedEntry = (customers) => {
    ...
    const allNames = customers.map(c => `${c.firstName} ${c.lastName}`);
    const uniqueNames = [...new Set(allNames)];
    
    return {
      'Destination': firstCustomer.email,
      'First Name': uniqueNames.length === 1 ? firstCustomer.firstName : 'Multiple',
      'Last Name': uniqueNames.length === 1 ? firstCustomer.lastName : 'Accounts',
  ...
  const downloadCSV = (filename) => {
    ...
    try {
      ...
      if (link.download !== undefined) {
        ...
        // Save processed data to localStorage for future reference
        saveToLocalStorage('lastProcessedData', processedData);
        saveToLocalStorage('lastProcessingStats', duplicateStats);
      }
    } catch (error) {
  ...
  const createSampleData = () => {
    const sampleData = [
      ...
      {CUSTID: 9591949, FNAME: 'BOB', LNAME: 'WILSON', EMAIL: 'bob.wilson@test.org', LICENSE_TYPE: 'Combination', RES_FLAG: 'Non-Resident'}
    ];
              ...
              {processedData.length > 0 && (
                <div>
                  <div className="bg-green-50 p-4 rounded-lg mb-4">
                    <CheckCircle className="inline h-5 w-5 text-green-500 mr-2" />
                    <span className="text-green-700">
                      Processed {processedData.length} records ready for email campaign
                    </span>
                  </div>

                  <button
                    onClick={() => downloadCSV('email_campaign_data.csv')}
            ...
            <div>
              ...
              {processedData.length > 0 && duplicateStats.totalRecords && (
                <div>
                  ...
                  <button
                    onClick={() => downloadCSV('deduplicated_email_data.csv')}
                    className="bg-green-600 text-white px-4 py-2 rounded-lg hover:bg-green-700 mb-4 flex items-center"
                  >
                    <Download className="mr-2 h-4 w-4" />
          ...
          <ol className="list-decimal list-inside space-y-2 text-gray-700">
            ...
            <li><strong>Personalization Strategy:</strong>
              <ul className="list-disc list-inside ml-6 mt-2 space-y-1">
                ...
                <li>Multiple accounts, same person: Combine information, list all Customer IDs</li>
                <li>Multiple accounts, different people: Use generic greeting ("Multiple Accounts") and list all relevant information</li>
              </ul>
            </li>
